#ifndef _BATTLE_RESULT_ENEMY_VIEW_H
#define _BATTLE_RESULT_ENEMY_VIEW_H
#define WIN32_LEAN_AND_MEAN

class CBattle_ResultEnemyView;

#include "systemUIDialog.h"

namespace battleResultEnemyViewNS
{
	const std::string ERROR_MESSAGE = "Battle Result EnemyView Initialize Failed";
	//====================================================
	const float VIEW_RELATE_X = -100.f;	// X = (g_fScreenWidth - 100 - VIEW_WIDTH)
	const float VIEW_RELATE_Y = -200.f;	// Y = ((g_fScreenHeight / 2) - (VIEW_HEIGHT / 2))
	const float VIEW_WIDTH = 300;
	const float VIEW_HEIGHT = 400;
	//====================================================
	const std::string VIEW_GUIDE_TITLE_PLAYER = "COMPUTER";
...
class CBattle_ResultEnemyView : public SystemUIDialog
{
private:
	RECT		m_rcViewTitle;
...
#endif // !_BATTLE_RESULT_ENEMY_VIEW_H